---
title: Architecture
description: How marky's mdast lint pipeline works under the hood.
---
...
marky is built on the [unified](https://unifiedjs.com/) ecosystem. Every lint pass follows the same five-step pipeline:
...
marky/
  packages/
    ...
    │ ├─ config.ts      loadConfig (jiti-based marky.config.ts loader)
    ...
    │
    cli/                @marky/cli
    │ ├─ cli.ts         marky lint / marky lsp / marky migrate
    │ ├─ migrate.ts     marky migrate (markdownlint → marky.config.ts)
    │ └─ reporters/
    │    ├─ pretty.ts   terminal output
    │    └─ json.ts     JSON output
    │
    plugin-mermaid/     @marky/plugin-mermaid
    │ └─ index.ts       remarkLintMermaid (@mermaid-js/parser)
    │
    compat-markdownlint/ @marky/compat-markdownlint
    ...
    │
    vscode/             @marky/vscode
      └─ extension.ts   VS Code extension (LanguageClient → @marky/lsp)
```

## Config loading

`marky.config.ts` is loaded at runtime using [jiti](https://github.com/unjs/jiti), which evaluates TypeScript without a compilation step. Config discovery walks up the directory tree from the linted file's location until it finds a `marky.config.ts` or reaches the filesystem root.

<Aside type="note">
  Config is cached per workspace root. The LSP server additionally watches
  `marky.config.ts` for changes and invalidates the cache automatically.
</Aside>

## Mermaid validation

The `@marky/plugin-mermaid` plugin takes a different approach from the text-based rules: it passes the raw content of each `mermaid` code block to `@mermaid-js/parser`, which uses a Langium-based grammar for accurate syntax validation.
...
Editor (VS Code, Neovim, Zed…)
  ...
  @marky/lsp server
   ...
   ├─ fs.watch watcher         ← hot-reload marky.config.ts
   ├─ Debounce timer (300ms)   ← onDidChange validation
   └─ validateMarkdown()       ← @marky/core lintString
        │  Diagnostic[]
        ▼
