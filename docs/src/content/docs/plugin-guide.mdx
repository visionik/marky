...
description: How to write a custom marky lint rule using unified-lint-rule and register it in marky.config.ts.
---

import { Steps, Aside } from '@astrojs/starlight/components'

marky rules are [unified/remark plugins](https://unifiedjs.com/). Any plugin that calls `file.message()` on a VFile is a valid marky rule.
...
- **`origin`** — `namespace:rule-name`. marky surfaces this as `ruleId` in violations (e.g. `my-plugin:my-rule`).
...
marky supports the unified `[plugin, options]` tuple convention:
...
```ts
// marky.config.ts
import type { MarkyConfig, Fixer } from '@marky/core'
import { myRule } from './my-rule.js'
...
marky uses the full [mdast](https://github.com/syntax-tree/mdast) specification. Common node types:
...
Use `lintString` from `@marky/core` to test rules without the CLI:
