...
#### Auto-fix support (`@crackdown/core` + `@crackdown/cli`)
...
- `MarkyConfig.fixers?: Fixer[]` — register fixers alongside plugins in `crackdown.config.ts`
- `crackdown lint --fix` — applies fixers and rewrites files in place; reports per-file fix counts
- `crackdown lint --fix --dry-run` — shows what would change without writing to disk
- Exit code reflects remaining error-severity violations after fixing

#### Bundled rules (`@crackdown/core`)
...
- Getting Started: install, 60-second usage, `crackdown.config.ts` example, package table
...
- API Reference: all `@crackdown/core` types (`MarkyConfig`, `LintResult`, `LintViolation`, `FixResult`, `Fixer`, `PluginEntry`) and functions
...
- Deployment URL: `https://visionik.github.io/crackdown/`

#### LSP server + VS Code extension
- `@crackdown/lsp` package — LSP server using `vscode-languageserver`
...
- Config hot-reload: watches `crackdown.config.ts` via `fs.watch`, invalidates cache on change
- `codeActionProvider` stub: surfaces violations as QuickFix actions
- `@crackdown/vscode` package — VS Code extension stub using `vscode-languageclient`, spawns `@crackdown/lsp` over stdio
- `crackdown lsp` CLI command — starts the LSP server over stdio for Neovim, Zed, Helix, etc.

#### Rule porting tool (`@crackdown/cli` + `@crackdown/compat-markdownlint`)
- `crackdown migrate [config-path]` — reads `.markdownlintrc` / `.markdownlint.json`, classifies each rule as supported/unsupported/disabled, writes a ready-to-use `crackdown.config.ts` with imports pre-wired
...
- New rules in `@crackdown/compat-markdownlint`:
  - `md001Rule` — heading levels increment by one
  - `md022Rule` — blank lines around headings
- `@crackdown/compat-markdownlint` `loadMarkdownlintConfig` now maps MD001, MD009, MD010, MD022, MD013, MD041
...
- `loadConfig(cwd)` — walks directory tree, loads `crackdown.config.ts` at runtime via jiti
...
#### `@crackdown/cli`
- `crackdown lint <glob|dir|->` CLI with `--format pretty|json` and `--config <path>` flags
...
- `crackdown.config.ts` example at repo root
